...
using InvalidVersionInfoException = MessagingToolkit.QRCode.ExceptionHandler.InvalidVersionInfoException;
...
namespace MessagingToolkit.QRCode.Codec.Reader.Pattern
{
	...
	public class FinderPattern
	{
		...
		internal static Line[] findLineAcross(bool[][] image)
		{
			...
			canvas.drawLines(foundLines, MessagingToolkit.QRCode.Codec.Util.Color_Fields.LIGHTGREEN);
			return foundLines;
		}
		...
		internal static Point[] getCenter(Line[] crossLines)
		{
			...
			{
				canvas.drawPolygon(foundPoints, MessagingToolkit.QRCode.Codec.Util.Color_Fields.RED);
				return foundPoints;
			}
			else
		...
		internal static int calcExactVersion(Point[] centers, int[] angle, int[] moduleSize, bool[][] image)
		{
			...
			canvas.drawPoints(points, MessagingToolkit.QRCode.Codec.Util.Color_Fields.RED);
